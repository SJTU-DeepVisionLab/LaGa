...
class FeatureGaussianModel:
    ...
    def change_to_segmentation_mode(self, training_args = None, target = 'coarse_seg_everything', fixed_feature = False, adjustable = False, opacity_adjustable = False):
        ...
        if training_args is not None:
            ...
            if not fixed_feature:
                ...
                if not adjustable and opacity_adjustable:
    ...
    def segment(self, mask=None):
        ...
        self.old_xyz = self._xyz
        self.old_point_features= self._point_features
        self.old_opacity = self._opacity
        self.old_scaling = self._scaling
        self.old_rotation = self._rotation
    ...
    def roll_back(self):
        try:
            self._xyz = self.old_xyz

            self._point_features = self.old_point_features
            self._opacity = self.old_opacity
            self._scaling = self.old_scaling
            self._rotation = self.old_rotation
        except:
    ...
    def get_bilateral_filtered_point_features(self, K = 16, dropout = 0.5, point_rgb = None, spatial_sigma=0.5, range_score_sigma=0.5, range_color_sigma=0.5):
        if K <= 1:
            return self._point_features
        ...
        if dropout > 0 and dropout < 1:
            real_K = int(K*dropout)
            # calculate no grad weight
            with torch.no_grad():
                select_point = torch.randperm(K)[ : real_K]
                select_idx = self.feature_smooth_map["m"][:, select_point]

        else:
            real_K = K
            with torch.no_grad():
                select_idx = self.feature_smooth_map["m"]

        xyz_expanded = self._xyz.unsqueeze(1).expand(-1, real_K, -1)   # (N, K, 3)
        features_expanded = self._point_features.unsqueeze(1).expand(-1, real_K, -1)       # (N, K, C)
        colors_expanded = point_rgb.unsqueeze(1).expand(-1, real_K, -1)   # (N, K, 3)

        neighbor_xyz = self._xyz[select_idx, :]       # (N, K, 3)
        neighbor_features = self._point_features[select_idx, :]       # (N, K, C)
        neighbor_colors = point_rgb[select_idx, :]       # (N, K, 3)

        spatial_distance = torch.norm(neighbor_xyz - xyz_expanded, dim=2)  # (N, K)
        spatial_weight = torch.exp(- (spatial_distance ** 2) / (2 * spatial_sigma ** 2))  # (N, K)

        feature_difference = torch.norm(features_expanded - neighbor_features, dim=2) # (N, K)
        range_feature_weight = torch.exp(- (feature_difference ** 2) / (2 * range_score_sigma ** 2))  # (N, K)
        ...
        range_weight = range_feature_weight * range_color_weight  # (N, K)
        weights = spatial_weight * range_weight  # (N, K)

        # 归一化权重
        weights_normalized = weights / (torch.sum(weights, dim=1, keepdim=True) + 1e-8)  # (N, K)

        ret = (weights_normalized.unsqueeze(-1).detach() * self._point_features[select_idx, :]).sum(dim = 1)
    ...
    @torch.no_grad()
    def save_ply(self, path, smooth_weights = None, smooth_type = None, smooth_K = None, generator = None, point_rgb = None):
        ...
        if smooth_weights is not None:
            ...
            f = self.get_bilateral_filtered_point_features(K=smooth_K, dropout=-1, point_rgb=point_rgb).detach().contiguous().cpu().numpy()
