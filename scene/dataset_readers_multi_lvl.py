...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None, mask_scale_folder = None, sample_rate = 1.0, allow_principle_point_shift = False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        features = torch.from_numpy(np.load(os.path.join(masks_folder, image_name.split('.')[0] + "_f.npy"))) if masks_folder is not None else None
        masks = torch.from_numpy(np.load(os.path.join(masks_folder, image_name.split('.')[0] + "_s.npy"))) if masks_folder is not None else None
...
def fetchPly(path, only_xyz=False):
    ...
    if {'red', 'green', 'blue'}.issubset(vertices.data.dtype.names) and not only_xyz:
        colors = np.vstack([vertices['red'], vertices['green'], vertices['blue']]).T / 255.0
    else:
        colors = None
    if {'nx', 'ny', 'nz'}.issubset(vertices.data.dtype.names) and not only_xyz:
...
# From OpenGaussian
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", need_features=False, need_masks=False):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames), desc="load images"):
            ...
            language_feature_path = os.path.join(path, "language_features")
            # language_feature_path = os.path.join(path, "language_features_langsplat")

            language_feature_path = language_feature_path if os.path.exists(language_feature_path) else os.path.join(path, "language_features/")

            mask_seg_path = os.path.join(language_feature_path, frame["file_path"].split('/')[-1] + "_s.npy")
            mask_feat_path = os.path.join(language_feature_path, frame["file_path"].split('/')[-1] + "_f.npy")
            if os.path.exists(mask_seg_path) and need_masks:
                sam_mask = torch.from_numpy(np.load(mask_seg_path))    # [level=4, H, W]
            else:
                sam_mask = None
            if os.path.exists(mask_feat_path) and need_features:
                mask_feat = torch.from_numpy(np.load(mask_feat_path))  # [num_mask, dim=512]
            else:
                mask_feat = None
            # modify -----
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, cx=cx, cy=cy, image=image, 
                            masks=sam_mask, features=mask_feat, mask_scales=None,
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", need_features=False, need_masks=False):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension, need_features, need_masks)
    test_cam_infos = []
    # print("Reading Test Transforms")
    # test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension, need_features, need_masks)
