...
class GaussianModel:
    ...
    def training_setup_scannet(self, training_args):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def segment(self, mask=None):
        ...
        self.old_mask = self._mask

        self.old_features_dc = self._features_dc
        self.old_features_rest = self._features_rest
    ...
    def roll_back(self):
        try:
            ...
            self._mask = self.old_mask

            self._features_dc = self.old_features_dc
            self._features_rest = self.old_features_rest
