...
class OpenCLIPNetwork(nn.Module):
    ...
    def get_relevancy_cosine(self, embed: torch.Tensor, positive_id: int):
        phrases_embeds = torch.cat([self.pos_embeds, self.neg_embeds], dim=0)
        p = phrases_embeds.to(embed.dtype)  # phrases x 512
        output = torch.mm(embed, p.T)  # rays x phrases
        positive_vals = output[..., positive_id : positive_id + 1]  # rays x 1
        ...
        lowest_score = softmax[..., 0].min(dim=1)[0]
        return lowest_score, positive_vals, negative_vals

    
    def filter_mask(self, masked_features) -> torch.Tensor:  
        masked_features = masked_features.cuda()      
        sigma = torch.var(torch.mm(masked_features, self.neg_embeds.T), dim=1) 
        flags = [1 for i in range(sigma.shape[0])]
...
def load_clip(config = OpenCLIPNetworkConfig):
    return OpenCLIPNetwork(config)