...
def render_contrastive_feature(viewpoint_camera, pc : FeatureGaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, norm_point_features = False, smooth_type = None, smooth_weights = None, smooth_K = 16, point_colors = None, multi_lvl_norm = False, multi_lvl_dim = [16,8,8], precomputed_features = None):
    """
    Render the scene. 
    ...
    if precomputed_features is None:
        if smooth_type is None:
            ...
            colors_precomp = pc.get_bilateral_filtered_point_features(K = smooth_K, dropout=0.5, point_rgb=point_colors)
        
        if norm_point_features and not multi_lvl_norm:
            colors_precomp = colors_precomp / (colors_precomp.norm(dim=1, keepdim=True) + 1e-9)
        elif multi_lvl_norm:
            normed_multi_lvl_features = []
            dim_pre = 0
            for dim in multi_lvl_dim:
                normed_multi_lvl_features.append(colors_precomp[:, dim_pre:dim_pre+dim] / (colors_precomp[:, dim_pre:dim_pre+dim].norm(dim=1, keepdim=True) + 1e-9))
                dim_pre += dim
            colors_precomp = torch.cat(normed_multi_lvl_features, dim=1)
    else:
        colors_precomp = precomputed_features.cuda()
