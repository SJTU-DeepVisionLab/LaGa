...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, segment : bool = False, target = 'scene', idx = 0, precomputed_mask = None):
    ...
    with torch.no_grad():
        ...
        if target == 'scene' or target == 'seg' or target == 'coarse_seg_everything':
            gaussians = GaussianModel(dataset.sh_degree)
        if target == 'coarse_seg_everything' or target == 'contrastive_feature':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', default='scene', const='scene', nargs='?', choices=['scene', 'seg', 'coarse_seg_everything', 'contrastive_feature'])
